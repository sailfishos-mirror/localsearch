...
static void map_to_tv_series (TrackerResource *resource,
                              const char      *property,
                              const char      *value,
                              MediaInfo       *ctx,
                              gpointer         user_data);

static void map_to_tv_season (TrackerResource *resource,
                              const char      *property,
                              const char      *value,
                              MediaInfo       *ctx,
                              gpointer         user_data);
...
static PropertyMapping video_mapping[] = {
	...
	{ "nmm:episodeNumber", Stream_General, L"Part", map_to_int },
	{ "nmm:series", Stream_General, L"Collection", map_to_tv_series },
	{ "nmm:isPartOfSeason", Stream_General, L"Season", map_to_tv_season },
                 ...
                 gpointer         user_data)
{
	...
	date = tracker_date_format_to_iso8601 (value, "%Y-%m-%d %H:%M:%S", tz);
	if (!date)
		date = tracker_date_format_to_iso8601 (value, "%Y", tz);
	if (!date)
		date = tracker_date_guess (value);
	if (date)
...
static void
map_to_tv_series (TrackerResource *resource,
                  const char      *property,
                  const char      *value,
                  MediaInfo       *ctx,
                  gpointer         user_data)
{
	g_autoptr (TrackerResource) tv_series = NULL;

	tv_series = tracker_extract_new_tv_series (value);
	tracker_resource_set_relation (resource, "nmm:series", tv_series);
	tracker_resource_add_uri (tv_series, "nmm:hasEpisode",
	                          tracker_resource_get_identifier (resource));
}

static void
map_to_tv_season (TrackerResource *resource,
                  const char      *property,
                  const char      *value,
                  MediaInfo       *ctx,
                  gpointer         user_data)
{
	g_autoptr (TrackerResource) tv_season = NULL;
	TrackerResource *tv_series = NULL;
	int64_t value_int;

	if (sscanf (value, "%" G_GINT64_MODIFIER "u", &value_int) != 1)
		return;

	tv_series = tracker_resource_get_first_relation (resource, "nmm:series");
	if (!tv_series)
		return;

	tv_season = tracker_extract_new_tv_season (tracker_resource_get_identifier (tv_series),
	                                           value_int, tv_series);
	tracker_resource_set_relation (resource, "nmm:isPartOfSeason", tv_season);
	tracker_resource_add_uri (tv_season, "nmm:hasSeasonEpisode",
	                          tracker_resource_get_identifier (resource));
}
